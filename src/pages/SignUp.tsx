...
const SignUp = () => {
      ...
      .then(() => {
        ...
        googleObj.accounts.id.initialize({
          ...
          callback: async (response: any) => {
            ...
            const result = await signInWithGoogle(response.credential, 'signup');
            setLoading(false);

            if (result.success) {
              toast({ title: 'Account ready', description: 'Signed up with Google successfully.', variant: 'success' });
              navigate(getPostAuthRedirect());
            } else {
  ...
  const handleGoogleSignUpFallback = () => {
    if (!googleReady) {
      toast({
        ...
        <Card className="w-full p-5 sm:p-7 shadow-xl border-2 border-[#2f7656] rounded-3xl">
          <div className="flex flex-col items-center gap-4 sm:gap-5">
            <div className="text-center w-full">
              <h1 className="text-2xl sm:text-3xl font-bold">Signup</h1>
              <p className="text-muted-foreground mt-2">Create your account with Google</p>
            </div>
